...
function AnaVanetMap() {
    ...
    this.cpuGraphMode   = true;
    this.memGraphMode   = true;
    ...
    this.progressSlideBar.slider({slide: function(event, ui) {_self.getProgressSliderBar();}});
    ...
    this.psusage = true;  // Hardware information extension from ps (process status)
}
...
AnaVanetMap.prototype = {
    ...
    initInfoArea : function() {
        ...
        this.showAllNodesData( firstData.nodes );
    ...
    initNodeLink : function() {
        ...
        for( var idx = 0; idx < firstData.nodes.length; idx++ ) {
            ...
            = new google.maps.Marker({
                                     ...
                                     map : this.map });
            this.nodeClicked( this.nodeMarkers[ node.node_id ], node );
        }
    ...
    //------------------------------
    initVersionCompatibility : function() {
        if (this.exp.data[0].nodes[0].cpuUtime != null){
            this.psusage = true;
        }else{
            this.psusage = false;
        }
    ...
    //------------------------------
    switchCpuGraph : function() {
        this.cpuGraphMode = !this.cpuGraphMode;
        this.redrawCpuBtn();
    },
    ...
    //------------------------------
    switchMemGraph : function() {
        this.memGraphMode = !this.memGraphMode;
        this.redrawMemBtn();
    },
    ...
    removeAllOverlayInfo : function() {
        ...
        this.reqPDRGraphMode   = false;
        this.repPDRGraphMode   = false;
        this.cpuGraphMode   = false;
        this.memGraphMode   = false;
    ...
    drawDisplay : function( data, timeDir ) {
        ...
        if( !$.isEmptyObject( this.graph ) ) {
            for( var key in this.graph ) {
                this.graph[key].createGraphAt( this.count, 'left' );
            }
        }
    ...
    //------------------------------
    showAllNodesData : function( nodes ) {
        for(var i = 0; i < nodes.length; i++) {
            this.showNodeData ( nodes[i]);
        }
    },

    //------------------------------
    showNodeData : function ( node ){
        var id   = '#info' + node.node_id;
        ...
        for( var i = 0; i < len; i++ ) {
            for( var j = 0; j < linkLen; j++ ){
                reqPdrData[ this.exp.data[0].links[j].name ].push(
                                                                  [i, this.exp.data[i].links[j].req_pdr * 100] );
            }
        }
        this.graph.req_pdr = new AnaVanetGraph(  {title : 'Req_PDR', plots : reqPdrData } );
        this.graph.req_pdr.createGraphAt( this.count, 'left'  );
    },
        ...
        for( var i = 0; i < len; i++ ) {
            for( var j = 0; j < linkLen; j++ ){
                resPdrData[ this.exp.data[0].links[j].name ].push(
                                                                  [i, this.exp.data[i].links[j].res_pdr * 100] );
            }
        }
        this.graph.res_pdr = new AnaVanetGraph(  {title : 'Res_PDR', plots : resPdrData } );
        this.graph.res_pdr.createGraphAt( this.count, 'left'  );
    },
    ...
    createJitterGraph : function() {
        ...
        this.graph.jitter.createGraphAt( this.count, 'left'  );
    },
    ...
    createRttGraph : function() {
        ...
        this.graph.rtt.createGraphAt( this.count, 'left'  );
    },
    //------------------------------
    createCpuGraph : function() {
        var cpuData = {};
        var len = this.exp.data.length;
        var nodeLen = this.exp.data[0].nodes.length;
        for( var j = 0; j < nodeLen; j++ ) {
            cpuData[ this.exp.data[0].nodes[j].name ] = [];
        }
        for( var i = 0; i < len; i++ ) {
            for( var j = 0; j < nodeLen; j++ ){
                cpuData[ this.exp.data[0].nodes[j].name ].push([i, this.exp.data[i].nodes[j].cpuUtime] );
            }
        }
        this.graph.cpu = new AnaVanetGraph(  {title : 'CPU', plots : cpuData } );
        this.graph.cpu.createGraphAt( this.count, 'right'  );
    },
    
    //------------------------------
    createMemGraph : function() {
        var memData = {};
        var len = this.exp.data.length;
        var nodeLen = this.exp.data[0].nodes.length;
        for( var j = 0; j < nodeLen; j++ ) {
            memData[ this.exp.data[0].nodes[j].name ] = [];
        }
        for( var i = 0; i < len; i++ ) {
            for( var j = 0; j < nodeLen; j++ ){
                var mem_usage = (this.exp.data[i].nodes[j].memTotal - this.exp.data[i].nodes[j].memFree) / this.exp.data[i].nodes[j].memTotal;

                memData[ this.exp.data[0].nodes[j].name ].push([i, mem_usage *100] );
            }
        }
        this.graph.mem = new AnaVanetGraph(  {title : 'MEM', plots : memData } );
        this.graph.mem.createGraphAt( this.count, 'right'  );
    },
    ...
    //------------------------------
    nodeClicked : function ( marker, node ) {
        google.maps.event.addListener(marker, 'click', function() {
                                      console.log(node.node_id);
                                      AnaVanetMap.prototype.switchCpuGraph();
                                      AnaVanetMap.prototype.switchMemGraph();
                                      });
...
AnaVanetGraph.prototype = {
    ...
    //------------------------------
    createGraphAt : function(t, position) {
        if( this.graphCont === null ) {
            this.createGraphCont(position);
        }
    ...
    //------------------------------
    createGraphCont : function(position) {
        this.graphCont = $('<div>').addClass('graphContainer');
        
        var dataInfo = $('<div></div>').attr('id', 'graph' + this.data.title);
        dataInfo.addClass('dragGraphAreaDefault');
        if (position == 'right'){
            dataInfo.addClass('dragGraphAreaRight');
        }
