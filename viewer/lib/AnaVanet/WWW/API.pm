...
sub rssi_heatmap {
    my $self    = shift;
    my $q       = $self->q;
    my $rssi_id = $q->param('rssi') || return;
    ...
    print $q->header(-type => "text/html; charset=utf-8");
    if( _is_rssi( $rssi_id ) ) {
        print $tx->render('template/rssi-heatmap-tx.html', $var);
    }
    else {
        print $tx->render('template/not-found-exp-tx.html', $var);
    }
...
sub get_rssi_data {
    my $self    = shift;
    my $q       = $self->q;
    my $rssi_id = $q->param('rssiData') || return;
    my $format  = $q->param('format') || 'json';
    my $rssi_data = _get_rssi_data({ rssi_id => $rssi_id, format => $format });
    print $q->header(-type => "text/plain; charset=utf-8");
    if( defined $rssi_data ){
        print $rssi_data;
    }
    else {
        print "Not found the experiment data [ID: $rssi_id]";
    }
}
...
#------------------------------
sub update_explanation {
    my $self        = shift;
    my $q           = $self->q;
    my $json        = $q->param('explanation');
    my $ref         = JSON::XS->new->decode( $json );
    my $exp_id      = $ref->{exp_id};
    my $explanation = $ref->{explanation};
    _update_explanation( $ref );
    print $q->header(-type => 'text/plain; charset=utf8');
    print JSON::XS->new->encode( {status => 'ok', exp_id => $exp_id } );
}
...
sub _get_rssi_data {
    my $param_ref = shift;
    my @json_files = glob("data/rssi/*.json");
    my ($target_file) = grep{ /$param_ref->{'rssi'}_/ } @json_files;
    return unless $target_file;
...
sub _get_all_exp_info {
    ...
    for my $json_file (@json_files) {
        my $explanation = _get_explanation( $json_file );
        
        $json_file =~ m{^$dir/(\d+)_([^_]+_[^_]+)_(\d+-\d+-\d+).json$};
        #my ($name, $time) = $json_file =~ m{^$dir/(\d+)_([^_]+_[^_]+)_(\d+-\d+-\d+).json$};
        push @all_exp_info, { exp_id => $1, name => $2, date => $3, explanation => $explanation };
...
#------------------------------
sub _update_explanation {
    my $arg_ref         = shift || return;
    my $exp_id      = $arg_ref->{exp_id};
    my $explanation = $arg_ref->{explanation};
    return unless _is_exp( $exp_id );
    ...
    my $new_json = JSON::XS->new->encode( $ref );
    my $temp_file = sprintf("data/json/_%s_temp.json", $exp_id);
    open my $fhw, ">", $temp_file;
    print $fhw $new_json;
    close $fhw;
    
    File::Copy::move( $temp_file, $target_file );
}
...
sub _is_rssi {
    my $rssi_id = shift;
    my @json_files = glob("data/rssi/*.json");
    (my $target_file) = grep{ /(?:$rssi_id)_/ } @json_files;
    return unless $target_file;
